...
if($result->num_rows > 0) { 
 ...
 while($row = $result->fetch_array()) {
	...
	if($row[3] == 1){
		$status = "<label class='label label-success'>Administrator</label>";//$activeBrands = "<label class='label label-success'>Available</label>";
	}else {
		$status = "<label class='label label-success'>User</label>";
	}
