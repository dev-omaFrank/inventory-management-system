...
function getProductData(row = null) {
  if (row) {
    ...
    if (productId == "") {
      ...
      // Call the fetchSelectedProduct function
      fetchSelectedProduct(productId, row);
    }
  } else {
...
// Define fetchSelectedProduct outside the if-else block
async function fetchSelectedProduct(productId, row) {
  try {
    ...
    const requestOptions = {
      ...
      body: `productId=${encodeURIComponent(productId)}`, // Corrected body format
    };

    // Send the request and wait for the response
    const response = await fetch('php_action/fetchSelectedProduct.php', requestOptions);
    ...
    document.getElementById("rate" + row).value = data.product_price;
    document.getElementById("rateValue" + row).value = data.product_price;

    document.getElementById("quantity" + row).value = 1;
    document.getElementById("available_quantity" + row).textContent = data.product_quantity;

    const total = (Number(data.product_price) * 1).toFixed(2);
    document.getElementById("total" + row).value = total;
    document.getElementById("totalValue" + row).value = total;

    // Call the subAmount function
    subAmount();
  } catch (error) {
    console.error('Error:', error);
    throw new Error("Could not send data to fetch selected product");
...
function subAmount() {
  ...
  var discount = $("#discount").val();
  if (discount) {
    var discountedPrice = Number(discount/100) * Number($("#totalAmount").val())
    var grandTotal = Number($("#totalAmount").val()) - Number(discountedPrice);
